...
func _process(_delta: float) -> void:
	# Open save menu with action 'menu_guardado_save' (map to U)
	if Input.is_action_just_pressed("menu_guardado_save"):
		_open_menu("save")
	# Open load menu with action 'menu_guardado_load' (map to I)
	if Input.is_action_just_pressed("menu_guardado_load"):
		_open_menu("load")

func _open_menu(new_mode: String) -> void:
	# If menu is open close it (toggle). Otherwise instantiate and open in new_mode.
